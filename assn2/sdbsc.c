...
int get_student(int fd, int id, student_t *s){
	 if (fd == -1) {
 	 ...
 	 off_t newOffset = id * STUDENT_RECORD_SIZE;
	
	 off_t lseekResult = lseek(fd, newOffset, SEEK_SET);
	 if (lseekResult < 0) {
		return ERR_DB_FILE;
	 }
...
int add_student(int fd, int id, char *fname, char *lname, int gpa){
	 student_t newStudent = EMPTY_STUDENT_RECORD;
	 off_t newOffset = id * STUDENT_RECORD_SIZE;
	 ...
	 student_t destinationRecord = EMPTY_STUDENT_RECORD;
	 ssize_t bytesRead = read(fd, &destinationRecord, STUDENT_RECORD_SIZE);
	 if (bytesRead < 0) {
	 	printf(M_ERR_DB_READ);
		return ERR_DB_FILE;
	 }

	 if (memcmp(&destinationRecord, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) != 0) {
...
int del_student(int fd, int id){
	 ...
	 lseekResult = lseek(fd, newOffset, SEEK_SET);
	 if (lseekResult < 0) {
	 	printf(M_ERR_DB_READ);
		return ERR_DB_FILE;
	 }

	 student_t newEmptyRecord = EMPTY_STUDENT_RECORD;
	 ssize_t bytesWritten = write(fd, &newEmptyRecord, STUDENT_RECORD_SIZE);
	 if (bytesWritten < 0) {
	 	printf(M_ERR_DB_WRITE);
		return ERR_DB_FILE;
	 }
	 ...
	 fsync(fd);
    read(fd, &destinationRecord, STUDENT_RECORD_SIZE);
...
int count_db_records(int fd){
	 ...
	 while (bytesRead != 0) {
	 	...
	 	destinationRecord = EMPTY_STUDENT_RECORD;
	 	bytesRead = read(fd, &destinationRecord, STUDENT_RECORD_SIZE);
	 	if (bytesRead < 0) {
	 		printf(M_ERR_DB_READ);
			return ERR_DB_FILE;
	 	}
