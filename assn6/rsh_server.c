...
int exec_client_requests(int cli_socket) {
    ...
    while(1) {
		 ...
		 int totalBytes = 0;
		 while (bytesRead > 0) {
		  	bytesRead = recv(cli_socket, io_buff+totalBytes, sizeof(io_buff), 0);
		  	if (bytesRead < 0) {
			  	perror("recv");
			  	return ERR_RDSH_COMMUNICATION;
		  	}
			...
			if ((char) io_buff[totalBytes - 1] == RDSH_EOF_CHAR) {
				io_buff[totalBytes - 1] = '\0';
				break;
			}
		 }
		 ...
		 if (rc == WARN_NO_CMDS) {
			 send_message_string(cli_socket, "warning no cmds\n"); 
		 } else if (rc == ERR_CMD_OR_ARGS_TOO_BIG) {
          send_message_string(cli_socket, "command too big\n");
       } else if (rc == OK) {
			 ...
			 if (cmd_rc == BI_CMD_EXIT) { 
				 ...
				 close(cli_socket);
				 return OK; 
			 } else if (cmd_rc == BI_CMD_STOP_SVR) {
				 send_message_eof(cli_socket);
				 close(cli_socket);
				 return OK_EXIT;
             ...
             pid_t supervisor = fork();	  
		 		 if (supervisor == -1) {
					 send_message_string(cli_socket, "supervisor error\n");
                exit(EXIT_FAILURE);
             }
             if (supervisor == 0) {
...
int send_message_string(int cli_socket, char *buff){
    int send_len = strlen(buff) + 1;
	 ...
	 while (total_sent < send_len) {
    	total_sent += send(cli_socket, buff+total_sent, send_len-total_sent, 0);
	 }

    if (total_sent != send_len){
        return ERR_RDSH_COMMUNICATION;
    }
...
int rsh_execute_pipeline(int cli_sock, command_list_t *clist) {
    ...
    int  pids_st[clist->num];         // Array to store statuses 
    int exit_code;
