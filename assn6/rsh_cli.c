...
int exec_remote_cmd_loop(char *address, int port)
{
    ...
    while (loop) {
		 ...
		 if (strlen(cmd_buff) == 4 && strcmp("exit", cmd_buff) == 0) {
			 loop = 0;
		 } else if (strlen(cmd_buff) == 11 && strcmp("stop-server", cmd_buff) == 0) {
			 loop = 0;
		 } else if (strncmp("exit ", cmd_buff, 5) == 0 || strncmp("stop-server ", cmd_buff, 12) == 0) {
			 loop = 0;
		 }
		 ...
		 while (total_sent < send_len) {
			 total_sent += send(cli_socket, cmd_buff+total_sent, send_len-total_sent, 0);
		 }
		 ...
		 int bytesRead = 1;
		 while (bytesRead > 0) {
		 	bytesRead = recv(cli_socket, rsp_buff, sizeof(rsp_buff), 0);
			if (bytesRead < 0) {
				perror("recv");
				exit(1);
			}
			...
			if ((char) rsp_buff[bytesRead - 1] == RDSH_EOF_CHAR) {
				rsp_buff[bytesRead - 1] = '\0';
				printf("%.*s", (int) bytesRead, rsp_buff);
				break;
			} 
